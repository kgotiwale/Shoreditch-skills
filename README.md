...
This is the single source of truth for studio skills. Everyone installs from here, so the work comes out consistent regardless of who ran it. That is the entire point. New studio skills belong in this repo, not in a client folder and not in a personal `~/.claude/skills/`.
...
Or describe the work ("audit this checkout flow on mobile", "UX review of these screens") and it triggers.

**Inputs:** screenshots, a live URL (driven via Playwright MCP), or Figma frames.

**Optional setup, only for the live-URL path:**
...
- 🟢 **Good.** Intuitive; the user acts without hesitation.
- 🟠 **Needs improvement.** The task completes, but costs a re-read or an extra step.
